...
import { DEFAULT_TRACKS, DEFAULT_AWARDS } from '@/lib/scoring';
...
export default function DashboardListPage() {
  ...
  async function loadDashboards() {
    ...
    // Get pending invites for this user with dashboard names
  ...
  async function createDashboard() {
    ...
    if (newTracks.length === 0) { alert('Add at least one track'); return; }
    const supabase = createClient();
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) return;
  ...
  const hasNone = ownedDashboards.length === 0 && sharedDashboards.length === 0 && pendingInvites.length === 0;
          ...
          <div className="relative bg-bg-card border border-border rounded-[16px] shadow-lg w-full max-w-[600px] mx-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="text-xs font-semibold text-text-secondary block mb-1.5">Awards</label>
                  <TrackEditor tracks={newAwards} onChange={setNewAwards} placeholder="Add an award..." addLabel="Add" emptyText="No awards yet." compact />
                </div>
              </div>
            </div>
